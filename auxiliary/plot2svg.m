function varargout = plot2svg(filename, id, debug, legendIcons, clippingMode, figureSize, pixelFileType, creditPrintBool)
  ...
  try
    ...
    PLOT2SVG_globals.ScreenPixelsPerInch = round(get(0, 'ScreenPixelsPerInch'));
  catch
...
function group = axes2svg(fid,id,ax,group,paperpos)
  ...
  if strcmp(get(ax,'Visible'),'on')
      ...
      if projection.xyplane
          ...
          tick_ratio = [2 1 3]; % [1 1 1]; % SA works better as for whatever reason tick lengths appear quite shorter for X and Z axes
          label_distance = -1.5*abs(ticklength);
...
function boundingBoxAxes = axchild2svg(fid,id,axIdString,ax,paperpos,axchild,axpos,groupax,projection,boundingBoxAxes)
  ...
  for i = length(axchild):-1:1
      ...
      elseif strcmp(get(axchild(i),'Type'),'rectangle')
          ...
          fprintf(fid,'<rect x = "%0.3f" y = "%0.3f" width = "%0.3f" height = "%0.3f" rx = "%0.3f" ry = "%0.3f" fill = "%s" stroke-linecap = "square" stroke-linejoin = "miter" stroke = "%s" stroke-width = "%0.3fpt" %s />\n',...
  ...
  for i = 1:size(xtot,1)
      ...
      if (any(isnan(x)) || any(isnan(y)))
          ...
          if ~strcmp(edgecolorname,'none')
              fprintf(fid,'<polygon fill = "none" stroke-linecap = "square" stroke-linejoin = "miter" stroke = "%s" stroke-width = "%0.3fpt" stroke-opacity = "%0.3f" %s points = "',...
...
function text2svg(fid,axpos,paperpos,id,ax,projection)
  ...
  if numel(textpos) > 2 && all(abs(textpos(3)-[-1,0,1]) > 1e-10) % 3D plot
...
function label2svg(fid,axpos,id,x,y,tex,align,angle,valign,lines,font_color)
  ...
  case 'cap', balign = 'central';
  case 'baseline', balign = 'central';
  case 'bottom', balign = 'text-after-edge';
  otherwise, balign = 'central';
  end
  switch lower(align)
