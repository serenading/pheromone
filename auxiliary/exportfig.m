...
for k = 1:2:length(paramPairs)
  ...
  switch (param)
   ...
   case 'boundscode'
    opts.boundscode = LocalCheckAuto(lower(value),auto.boundscode);
    if ~strcmp(opts.boundscode,{'internal','mcode'})
      error('BoundsCode must be ''internal'' or ''mcode''.');
    end
   case 'lockaxes'
...
try
  ...
  if ~isempty(allAxes)
    usemcode = 1;
    if strncmp(opts.format,'eps',3)
      if strcmp(opts.boundscode,'internal')
