...
%% script reads area and perimeter features files, normalises multi-worm blob features using single-worm features, and plots various visualisations of the data
...
areaCutOff = 7; % 5 seems good
perimeterCutOff = 3; % 2 or 2.5 seems good
saveResults = false;
...
for strainCtr = 1:length(strains)
    ...
    perimeterBinEdges = [1:0.15:19];
    areaBinEdges = [1:0.5:54];
    ...
    for fileCtr = 1:numFiles
        ...
        % normalise histogram bin count by probability and log transform
        perimeterHistCount.(strains{strainCtr})(fileCtr,:) = perimeterHistCount.(strains{strainCtr})(fileCtr,:)/numel(perimeterNorm.(strains{strainCtr}){fileCtr});
        areaHistCount.(strains{strainCtr})(fileCtr,:) = areaHistCount.(strains{strainCtr})(fileCtr,:)/numel(areaNorm.(strains{strainCtr}){fileCtr});
...
perimeter_x = perimeterBinEdges(1:end-1);
area_x = areaBinEdges(1:end-1);
...
ylim([0 0.2])
ylabel('Probability')
perimeterDistFig = gcf;
figurename = ['figures/perimeterDistFig'];
if saveResults
    exportfig(perimeterDistFig,[figurename '.eps'],exportOptions)
    system(['epstopdf ' figurename '.eps']);
    system(['rm ' figurename '.eps']);
end
...
ylabel('Probability')
areaDistFig = gcf;
figurename = ['figures/areaDistFig'];
if saveResults
    exportfig(areaDistFig,[figurename '.eps'],exportOptions)
    system(['epstopdf ' figurename '.eps']);
    system(['rm ' figurename '.eps']);
end
...
xlim([1 7])
ylim([10e-4 1])
figurename = 'figures/perimeter_LogScale';
if saveResults
...
xlim([1 15])
ylim([10e-4 1])
figurename = 'figures/area_LogScale';
if saveResults
